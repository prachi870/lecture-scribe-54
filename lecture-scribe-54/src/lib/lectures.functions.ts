...
async function fetchYouTubeTranscript(videoId: string): Promise<{ text: string; title: string }> {
  ...
  if (!capRes.ok) {
    throw new Error(`YouTube caption fetch failed with status ${capRes.status}. The video may be restricted or region-locked.`);
  }
  const capRaw = await capRes.text().catch(() => "");
  if (!capRaw || !capRaw.trim()) {
    throw new Error("The YouTube transcript service returned an empty response. The captions for this video may be restricted or disabled.");
  }
